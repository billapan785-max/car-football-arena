...
export default function App() {
  ...
  const [screen, setScreen] = useState<'loading' | 'login' | 'lobby' | 'playing' | 'loading_match'>('loading');
  const [renderedScreen, setRenderedScreen] = useState<'loading' | 'login' | 'lobby' | 'playing' | 'loading_match'>('loading');
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      if (currentUser) {
        // If we collected a user name during login/signup, update the profile
        ...
        // Assuming we store age somewhere or just require it for validation
      }
      setUser(currentUser);
  ...
  const [bgImage, setBgImage] = useState('/splash.jpeg');
  ...
  const [selectedStadium, setSelectedStadium] = useState<'emerald' | 'cyber'>(() => {
    const saved = localStorage.getItem('selectedStadium');
    return (saved === 'cyber' || saved === 'emerald') ? saved : 'emerald';
  });
  ...
  const [settingsTab, setSettingsTab] = useState<'in-match' | 'display' | 'sound' | 'recording' | 'notification' | 'account'>('in-match');
  ...
  const [qualityPreset, setQualityPreset] = useState<'Low'|'Medium'|'High'|'Ultra'>('Low');
  ...
  // Derive list of owned car presets
  const ownedCarPresets = CAR_PRESETS.filter(car => ownedCarIds.includes(car.id));

  // Get currently selected car info
  const selectedCar = ownedCarPresets[selectedOwnedCarIndex] || ownedCarPresets[0] || CAR_PRESETS[2];
  ...
  useEffect(() => {
    ...
    imgBg.src = '/splash.jpeg';
    imgBg.onload = () => {
      setBgImage('/splash.jpeg');
      setHasBgError(false);
    };
    imgBg.onerror = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribeUsers = onSnapshot(q, (snapshot) => {
      ...
      snapshot.forEach((doc) => {
        const data = doc.data();
        if (data.uid !== uid && data.lastSeen > Date.now() - 30000) {
          users.push(data);
        }
      });
      
      // Inject fake bots to simulate online friends
      users = [
        ...users,
        { uid: 'bot_fake_1', displayName: 'GhostRider', email: 'ghost@test.com', status: 'online', statusText: 'In Lobby', lastSeen: Date.now() },
        { uid: 'bot_fake_2', displayName: 'SpeedyDemon', email: 'speedy@test.com', status: 'playing', statusText: 'In Match (2-1)', lastSeen: Date.now() },
        { uid: 'bot_fake_3', displayName: 'NoobMaster', email: 'noob@test.com', status: 'offline', statusText: 'Offline', lastSeen: 0 }
      ];
      ...
      try {
        ...
        querySnapshot.forEach((doc) => {
          ...
          if (data.uid !== user?.uid) {
            ...
            const matchesUid = data.uid?.toLowerCase() === cleanQuery;
  ...
  useEffect(() => {
    if (screen === 'lobby' && user) {
      ...
      if (lobbyId && user && lobbyId === user.uid) {
        deleteDoc(doc(db, 'lobby_teams', lobbyId)).catch(e => console.error("Error deleting lobby:", e));
        setLobbyId(null);
      }
  ...
  }, [renderedScreen, selectedStadium]);
  ...
  const handleLaunchMatch = async () => {
    ...
    // Choose stadium: Ranked is always Neon Cyberdome ('cyber'). Others select randomly between 'cyber' and 'emerald'.
    let chosenStadium: 'cyber' | 'emerald' = 'emerald';
    if (matchMode === 'ranked') {
      chosenStadium = 'cyber';
    } else {
      chosenStadium = Math.random() > 0.5 ? 'cyber' : 'emerald';
    }
      ...
      >
        <div id="login-buttons-container" style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', pointerEvents: 'auto' }}>
          ...
          <div style={{
            ...
            <button 
              id="facebook-login"
              onClick={() => handleLoginAttempt(() => signInWithPopup(auth, new FacebookAuthProvider()).catch(e => alert("Facebook Login Error: " + e.message)))}
              style={{ 
                width: '253px', 
                height: '42px', 
                background: '#1877f2', 
          ...
          {showMoreLoginOptions && (
            <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.8)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', zIndex: 100, pointerEvents: 'auto' }}>
              <div style={{ background: '#111827', padding: '30px', borderRadius: '12px', border: '2px solid #00d2ff', display: 'flex', flexDirection: 'column', gap: '15px', width: '300px' }}>
                ...
                <button 
                  onClick={() => handleLoginAttempt(() => signInWithPopup(auth, new GoogleAuthProvider()).catch(e => alert("Google Login Error: " + e.message)))}
          ...
          {showProfileModal && (
            <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.85)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: '20px', pointerEvents: 'auto' }}>
              <div style={{ background: '#111827', padding: '30px', borderRadius: '12px', border: '2px solid #00d2ff', display: 'flex', flexDirection: 'column', gap: '15px', width: '100%', maxWidth: '400px' }}>
                <h2 style={{ margin: 0, color: '#43f5ff', fontSize: '1.2rem', fontWeight: 900, textTransform: 'uppercase', textAlign: 'center' }}>
                  User Profile Setup
                </h2>
                
                <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                  <label style={{ color: 'white', fontSize: '0.9rem' }}>Display Name</label>
                  <input 
                    type="text"
                    value={profileNameInput}
                    onChange={e => setProfileNameInput(e.target.value)}
                    placeholder="Enter your display name"
                    style={{ padding: '10px', borderRadius: '6px', border: '1px solid #333', background: '#1f2937', color: 'white', outline: 'none' }}
                  />
                </div>

                <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                  <label style={{ color: 'white', fontSize: '0.9rem' }}>Age</label>
                  <input 
                    type="number"
                ...
                <div style={{ display: 'flex', gap: '10px', marginTop: '10px' }}>
                  <button 
                    ...
                    style={{ flex: 1, padding: '10px', background: 'transparent', color: '#8fa2c4', border: '1px solid #8fa2c4', borderRadius: '8px', cursor: 'pointer' }}
                  >
                    Cancel
                  </button>
                  <button 
                    ...
                    style={{ flex: 1, padding: '10px', background: '#00d2ff', color: '#000', borderRadius: '8px', border: 'none', fontWeight: 'bold', cursor: 'pointer' }}
                  >
                    Continue
                  </button>
                </div>
  ...
  if (renderedScreen === 'lobby') {
    const ourName = user?.displayName || user?.email?.split('@')[0] || 'User';
    const isSquadLobbyActive = matchTeams && (
      matchTeams.blue.filter(n => n !== '').length > 1 || 
    ...
    return (
      ...
      >
        ...
        <div style={{ position: 'absolute', inset: 0, zIndex: 1 }}>
          {isSquadLobbyActive ? (
            <SquadCanvas 
              blueNames={matchTeams?.blue || [ourName, '', '', '']}
        ...
        <div style={{ position: 'absolute', inset: 0, zIndex: 10, pointerEvents: 'none' }}>
          ...
          <div style={{ 
            ...
            <button
              ...
              style={{
                background: 'url(/modebutton.png) no-repeat center/100% 100%',
                width: '120px',
                height: '34px',
                color: 'white',
            ...
            <button 
              ...
              style={{ 
                width: '120px', 
                height: '34px', 
                background: 'url(/playbutton.png) no-repeat center/100% 100%',
          ...
          <div style={{ position: 'absolute', top: '110px', left: '356px', width: '130px', height: '40px', pointerEvents: 'auto' }}>
            <button 
              ...
              style={{ 
                width: '130px', 
                height: '40px', 
                background: 'url(/garagebutton.png) no-repeat center/cover',
                opacity: 1,
                outline: 'none',
                marginLeft: '-348px',
                marginTop: '6px',
          ...
          {/* Settings Button below Garage Button */}
          <div style={{ position: 'absolute', top: '160px', left: '356px', width: '130px', height: '40px', pointerEvents: 'auto' }}>
            <button 
              onClick={() => setActiveModal('settings')}
              className="hover:scale-105 transition-all relative overflow-hidden flex items-center justify-center gap-2"
              style={{ 
                width: '130px', 
                height: '40px', 
                ...
                textTransform: 'uppercase',
                cursor: 'pointer',
                border: '1.5px solid #43f5ff',
                borderRadius: '6px',
                boxShadow: '0 0 15px rgba(67, 245, 255, 0.5), inset 0 0 8px rgba(67, 245, 255, 0.25)',
                textShadow: '0 0 8px #43f5ff',
                outline: 'none',
                marginLeft: '-348px',
                marginTop: '6px'
              }}
            >
              <Settings className="w-4 h-4" style={{ animation: 'spin 8s linear infinite' }} />
              SETTINGS
            </button>
          </div>
              ...
              <div style={{ padding: renderedActiveModal === 'settings' ? '0' : '20px', maxHeight: renderedActiveModal === 'settings' ? '100vh' : '60vh', height: renderedActiveModal === 'settings' ? '100%' : 'auto', display: renderedActiveModal === 'settings' ? 'flex' : 'block', flexDirection: 'column', overflowY: 'auto', fontSize: '0.85rem', color: '#c4d1eb', lineHeight: '1.45' }}>
                ...
                {renderedActiveModal === 'car_shop' && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                    ...
                    <div style={{ background: 'rgba(67, 245, 255, 0.05)', border: '1px solid rgba(67, 245, 255, 0.25)', borderRadius: '10px', padding: '12px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div>
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                              ...
                              <div>
                                {isOwned ? (
                                  <button
                                    onClick={() => {
                                      // Set this car as active in the owned list!
                                  ...
                                  <button
                                    onClick={() => {
                                      if (coins >= car.price) {
                                        ...
                                        // Auto-select the newly purchased car!
                                        const updatedOwnedPresets = CAR_PRESETS.filter(p => p.id === car.id || ownedCarIds.includes(p.id));
                                        ...
                                        setActiveModal(null); // Close modal to show off the new car!
                                      } else {
                ...
                {renderedActiveModal === 'settings' && (
                  <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
                    ...
                    <div style={{ display: 'flex', gap: '16px', flex: 1, overflow: 'hidden' }}>
                      {/* SIDEBAR TABS */}
                      <div style={{ width: '150px', display: 'flex', flexDirection: 'column', gap: '2px', paddingRight: '8px', borderRight: '1px solid rgba(67, 245, 255, 0.1)' }}>
                        {[
  ...
  return (
    <div ref={mountRef} className="boostball-root">
      ...
      <div className="boostball-ui">
        {/* Cam controls */}
        <div style={{ position: 'absolute', top: 80, left: '50%', transform: 'translateX(-50%)', pointerEvents: 'auto', zIndex: 10 }}>
          <button data-cam-toggle style={{ background: 'rgba(0,0,0,0.7)', border: '1px solid #43f5ff', color: '#43f5ff', padding: '6px 16px', borderRadius: '20px', fontWeight: 'bold', fontSize: '12px', letterSpacing: '1px', textTransform: 'uppercase', cursor: 'pointer', boxShadow: '0 0 10px rgba(67, 245, 255, 0.3)' }}>
            View: Close
          </button>
        </div>
        ...
        {/* Boost Dash */}
        <div className="boostball-dash">
          <div style={{ width: 36, height: 36, borderRadius: '50%', background: '#ffd32a', gridRow: '1 / 3' }} />
          <div className="boost-shell">
            <i data-boost-bar style={{ transformOrigin: 'left center' }}></i>
          </div>
          <span data-boost-text>BOOST 100%</span>
        </div>
        ...
        {mobileControlsEnabled && (
          <div style={{ position: 'absolute', inset: 0, zIndex: 12, pointerEvents: 'none', opacity: mobileControlsOpacity / 100 }}>
            {[
              { id: 'steerLeft', label: '◀', color: '#43f5ff' },
              { id: 'steerRight', label: '▶', color: '#43f5ff' },
              { id: 'forward', label: '▲', color: '#10b981' },
              { id: 'reverse', label: '▼', color: '#ef4444' },
              { id: 'boost', label: '⚡', color: '#ffd32a' },
              { id: 'jump', label: '🚀', color: '#9d4edd' }
            ].map((btn) => {
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    cursor: 'pointer',
                    pointerEvents: 'auto',
                    boxShadow: `0 0 14px ${btn.color}60, inset 0 0 8px ${btn.color}30`,
