...
export default function SquadCanvas({
  ...
  useEffect(() => {
    ...
    const topSpotLight = new THREE.SpotLight(0xffffff, 5.0, 30, Math.PI / 4, 0.5, 1);
    topSpotLight.position.set(0, 10, 0);
    topSpotLight.castShadow = true;
    scene.add(topSpotLight);

    // Dynamic Fill Lights
    const blueFill = new THREE.PointLight(0x00aaff, 3.0, 25);
    blueFill.position.set(-8, 5, 3);
    scene.add(blueFill);

    const orangeFill = new THREE.PointLight(0xffaa00, 3.0, 25);
    orangeFill.position.set(8, 5, 3);
